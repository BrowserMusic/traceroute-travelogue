...
function prepJSON() {
  const path = [ashburn, wellington, whanganui, auckland, tasman, sydney1, perth, sydney2, cali, manhattan, toronto, ashburn, cville];
...
const path = {
  ...
  mutations: {
    ...
    updateScene(state, obj) {
      const scene = state.paths[state.currentPath][state.currentCity][obj.sceneIndex];
      state.paths[state.currentPath][state.currentCity][obj.sceneIndex] =
      {
        ...scene,
        ...obj.data
      };
  ...
  actions: {
    ...
    hubAlteration({ commit, getters }, obj) {
      let scene = getters.getSceneById(obj.id);
      commit("updateScene", {
